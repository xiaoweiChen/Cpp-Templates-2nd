...
\subsubsubsection{4.4.1\hspace{0.2cm}可变参的表达式}
...
\subsubsubsection{4.4.2\hspace{0.2cm}可变参的索引}
...
\subsubsubsection{4.4.3\hspace{0.2cm}可变参的类模板}
...
\subsubsubsection{4.4.4\hspace{0.2cm}可变参的推导策略}

推导策略(参见第2.9节)也可以是可变参的。例如，C++标准库定义了以下\texttt{std::arrays}的推导策略:
...
\subsubsubsection{4.4.5\hspace{0.2cm}可变参基类和using}
...
这里，首先定义了一个类\texttt{Customer}和一些独立的函数对象，用于哈希和比较\texttt{Customer}对象。

当将原始数组或字符串字面值传递给模板时，必须小心。如果模板参数声明为引用，则实参不会衰变，传入的"hello"参数为char const[6]类型。若因类型不同而传递不同长度的原始数组或字符串参数，这可能会成为一个问题。只有当按值传递实参时，类型才会衰变，因此字符串字面值转换为char const*。这将在第7章中详细讨论。

...
根据签名，第二个函数模板在(分别)用int[10]和char const[4]替换T时也会匹配，这些替换会导致返回类型T::size\_type中出现错误。因此，对于这些调用，第二个模板将直接忽略。
...
\subsubsubsection{8.4.1\hspace{0.2cm}表达式SFINAE与decltype}

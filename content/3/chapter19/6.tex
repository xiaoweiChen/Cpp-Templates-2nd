...
\subsubsubsection{19.6.1\hspace{0.2cm}检测成员类型}
...
作为开发者，我们在考虑领域“边缘”可能会出现的问题。对于像HasSizeTypeT这样的特征模板，引用类型可能会产生一些问题。例如，下面的代码工作很好:
...
\subsubsubsection{19.6.2\hspace{0.2cm}检测成员类型}
...
目前，在不使用宏的情况下，得到的最接近的结果是使用泛型Lambda，如第19.6.4节所示。
...
当\&T::Member无效时，使用SFINAE禁用偏特化。要使该结构有效，以下条件必须为真:
...
这里的挑战在于为条件检查定义一个有效的表达式，并使用decltype将其放置在SFINAE中，如果表达式无效，则会导致回退到主模板:
...
\subsubsubsection{19.6.4\hspace{0.2cm}使用泛型Lambdas检测成员}
...
为了能够使用通用泛型语法，将类型作为模板参数，可以再次定义额外的协助程序。例如:

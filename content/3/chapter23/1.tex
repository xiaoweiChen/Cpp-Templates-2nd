...
这个函数的实现在运行时可能不是最高效的(在运行时利用机器的特性通常会有很好的回报)，但因为是来执行编译时计算的，所以绝对效率不如可移植性重要。注意，在这个平方根示例中没有看到高级的“魔术模板”，只有函数模板的常见模板参数推导。代码是“普通的C++”，读起来并不困难。
...
元函数通过让匹配顶层数组情况的偏特化递归地“调用”元函数本身来执行其任务。

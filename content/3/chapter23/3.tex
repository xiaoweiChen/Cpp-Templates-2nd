...
这里的关键变化是IfThenElse模板的使用，该模板19.7.1节中介绍过。记住，IfThenElse模板是一个基于给定布尔常数在两种类型之间进行选择的工具。如果该常量为真，则第一个类型被类型别名为type;否则，Type代表第二种类型。并且，为类模板实例定义类型别名并不会导致C++编译器实例化该实例体。因此，当代码如下时
